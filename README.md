...
|                                 |                                                                                                                   |
|---------------------------------|-------------------------------------------------------------------------------------------------------------------|
| **SDK Filename**                | fraudforce-lib-release-3.1.0.aar                                                                                  |
| **Version**                     | 3.1.0                                                                                                             |
...
Version 3.1.0 of the iovation FraudForce SDK for Android supports Android 4.1 or higher.

## Installing the FraudForce SDK for Android

1.  Download iovation-android-sdk-3.1.0.zip from here: [iovation Mobile SDK for Android](https://github.com/iovation/deviceprint-SDK-Android). 

2.  Unzip iovation-android-sdk-3.1.0.zip.
...
3.  Depending on your IDE, do one of the following:
	...
	- If you are using __Android Studio with Gradle__, add the *fraudforce-lib-release-3.1.0.aar* file to your application module's libs directory. Then, edit the *build.gradle* file in order to add the libs directory as a flat-file repository to the `buildscript` and `repository` sections. This makes the fraudforce-lib-release-3.0.0.aar file accessible to Gradle.
		...
		```
		Also in the application module's `build.gradle` file, make sure that fraudforce-lib-release-3.1.0 is a compile-time dependency:
	
		```
        dependencies {
            compile fileTree(dir: 'libs', include: ['*.jar'])
            compile(name:'fraudforce-lib-release-3.1.0', ext:'aar')
        }
		```
...
This release introduces the ability for the android SDK to make a network call to iovation's service. This enables additional functionality in the FraudForce SDK, including:
...
1. Log into the Administration Console and download fraudforce-lib-release-3.1.0.zip from here: [iovation Mobile SDK for Android](https://github.com/iovation/deviceprint-SDK-Android).

2. In Android Studio, select File | Open or click **Open Existing Android Studio Project** from the quick-start screen.

3. From the directory where you unzipped fraudforce-lib-release-3.1.0.zip, open the **android-studio-sample-app** directory.

4. In the project navigation view, open `src/main/java/com/iovation/mobile/android/sample/MainActivity.java`

5. Right-click the file editing view and select _Run Main Activity_.
...
6. Select either an attached physical device, or an Android virtual device to run the app on. The app should now compile and launch.

7. When the app compiles successfully, you will see a view with a button that allows you to display a blackbox.
